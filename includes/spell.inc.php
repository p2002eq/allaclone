    ...
    switch ($spell["effectid$n"]) {
      case 3: // Increase Movement (% / 0)
        if ($max<0) { // Decrease
          ...
          if ($min!=$max) { print " by ".abs($min)."% (L$minlvl) to ".abs($max)."% (L$maxlvl)"; } else { print " by ".abs(100)."%"; }
        } else {
          print "Increase Movement";
          if ($min!=$max) { print " by ".$min."% (L$minlvl) to ".($max)."% (L$maxlvl)"; } else { print " by ".($max)."%"; }
        }
        break;
      case 11: // Decrease OR Inscrease AttackSpeed (max/min = percentage of speed / normal speed, IE, 70=>-30% 130=>+30%
        if ($max<100) { // Decrease
          ...
          if ($min!=$max) { print " by ".(100-$min)."% (L$minlvl) to ".(100-$max)."% (L$maxlvl)"; } else { print " by ".(100-$max)."%"; }
        } else {
          print "Increase Attack Speed";
          if ($min!=$max) { print " by ".($min-100)."% (L$minlvl) to ".($max-100)."% (L$maxlvl)"; } else { print " by ".($max-100)."%"; }
        }
        break;
      case 21: // stun 
        print $dbspelleffects[$spell["effectid$n"]];
        if ($min!=$max) { print " (".($min/1000)." sec (L$minlvl) to ".($max/1000)." sec (L$maxlvl))"; } else { print " (".($max/1000)." sec)"; }
        break;
      case 32: // summonitem
        print $dbspelleffects[$spell["effectid$n"]];
      ...
      case 100: // Increase Hitpoints v2 per tick
        print $dbspelleffects[$spell["effectid$n"]];
        if ($min!=$max) { print " by ".abs($min)." (L$minlvl) to ".abs($max)." (L$maxlvl) per tick (total ".abs($min*$duration)." to ".abs($max*$duration).")"; } else { print " by $max per tick (total ".abs($max*$duration).")"; }
        break;
      ...
      case 140: // Limit: Min Duration
        print $dbspelleffects[$spell["effectid$n"]];
        $min*=6; $max*=6;
        if ($min!=$max) { print " ($min sec (L$minlvl) to $max sec (L$maxlvl))"; } else { print " ($max sec)"; }
        break;
      case 143: // Limit: Min Casting Time
        print $dbspelleffects[$spell["effectid$n"]];
        $min*=6; $max*=6;
        if ($min!=$max) { print " (".($min/6000)." sec (L$minlvl) to ".($max/6000)." sec (L$maxlvl))"; } else { print " (".($max/6000)." sec)"; }
        break;
      ...
      default:
        ...
        if ($max<0) { $name=str_replace("Increase","Decrease",$name); }
        print $name;
        if ($min!=$max) { print " by $min (L$minlvl) to $max (L$maxlvl)"; } 
        else { if ($max<0) { $max=-$max; } print " by $max"; }
        break;
    }
	echo '<br>';
  }
